...
public:
    ...
    bool select_method(dcsp::dcsp_srv::Request &req, dcsp::dcsp_srv::Request &res);
            ...
            if (is_new_value_consistent) {
                ...
                for (it3 = lower_priority_agents.begin(); it3 != lower_priority_agents.end(); ++it3){
                    ...
                    if (lower_priority_agent == "firefly1") {
                     ...
                     dcsp::dcsp_srv srv;
                     srv.request.init_agent_identifier = "";
...
bool Agent::select_method(dcsp::dcsp_srv::Request &req, dcsp::dcsp_srv::Request &res){
    ...
    if(request.init){
        ...
        agent_init(request.agents, request.init_agent_identifier,request.init_point ,request.my_domain);
        
    }else if (request.ok){
        
        recieve_ok_msg(request.ok_agent_identifier, request.ok_value);
        return true;
    }else if (request.nogood){
        receive_nogood_msg(request.no_goods);
    }
...
int main(int argc, const char * argv[]) {
    ...
    ros::NodeHandle n;
    ros::ServiceServer service = n.advertiseService("dcsp_srv_1", Agent::select_method);
